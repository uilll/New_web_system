...
function send_sms_async($url, $params, $result = false)
{
    ...
    if ($result) {
        ...
        if (isset($params['encoding']) && $params['encoding'] == 'json') {
            curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json', 'Content-Length: '.strlen($new_post_string)]);
        } else {
            curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Length: '.$postLength]);
        }
