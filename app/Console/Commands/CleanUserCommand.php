...
class CleanUserCommand extends Command
{
    ...
    public function fire()
    {
        ...
        $users = User::where('loged_at', '<', Carbon::now()->subMonths(2))
        ...
        $users = User::leftJoin('devices', 'users.id', '=', 'devices.user_id')
        ...
        $users = User::where('subscription_expiration', '!=', '0000-00-00 00:00:00')
        ...
        $devices = Device::where('devices.updated_at', '<', Carbon::now()->subMonths(1))
            ->select('devices.*')
