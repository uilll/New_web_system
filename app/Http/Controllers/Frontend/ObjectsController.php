...
use Tobuli\Helpers\GeoLocation\GeoServices\GeoGeocodio;
...
class ObjectsController extends Controller {
				...
				foreach ($devices as $key => $device) {
					...
					if(isset($array["ignition"])){
						...
						if (!stristr($device->registration_number, "virtual") === false){ //strval(strpos($device->registration_number,"CRX3"))>="0"){
							if ($array["ignition"]=="true") $ignition_status = "Ign. ligada";
							else $ignition_status = "Ign. desligada";						
						}
						else{
					...
					if ($icon_color == 'green' || $icon_color == 'black') $shadow_area = 'on-line';
					else $shadow_area = 'Em sombra';
					$devices[$key]->shadow_area = $shadow_area;
					if ($device->active =="0") $active_gpswox = 'Inativo';
					else $active_gpswox = 'Ativo';
					...
					if ($speed_ != 0) $ignition_status .= " - Em movimento";
					else $ignition_status .= " - Parado";
	...
	public function traccer_route($id=null){
		...
		$coordenadas = array("lat" => $device->traccar->lastValidLatitude, "lon" => $device->traccar->lastValidLongitude, "juntas" => $device->traccar->lastValidLatitudet.",".$device->traccar->lastValidLongitude);
	...
	public function share_device(int $device_id){
		try{
			...
			if(empty($link)){
				...
				$data=array('user_id'=>$user_ok->id,"device_id"=>$device_id);
