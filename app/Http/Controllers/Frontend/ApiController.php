...
class ApiController extends Controller
{
	...
	$devices[$i]->traccar->server_time = date('d/m/Y H:i', strtotime('-3 hour', strtotime($devices[$i]->traccar->server_time)));
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('lat' => $latitude_x,'lng' => $longetude_x,'map' => 'CARSEG')));
		$data = curl_exec($ch);
		curl_close($ch);
...
public function obtemdispositivosapp($iduser,$latitudes,$longitudes)
	...
	for($i = 0; count($devices) > $i ; $i++){	
		...
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST,  2);
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('lat' => $devices[$i]->traccar->lastValidLatitude,'lng' => $devices[$i]->traccar->lastValidLongitude,'map' => 'CARSEG')));
		$data = curl_exec($ch);
		curl_close($ch);
		$data = json_decode($data);
    ...
    public function obtemposicaoatualapp($user_id, $device_id)
    {
		...
		if($item->lat !== null && $item->lat !== '' && $item->lng !== null && $item->lng !== '' ){
			...
			curl_setopt($ch, CURLOPT_SSL_VERIFYHOST,  2);
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('lat' => $item->lat,'lng' => $item->lng,'map' => 'CARSEG')));
			$data = curl_exec($ch);
			curl_close($ch);
			$data = json_decode($data);
...
$devices[$i]->traccar->server_time = date('d/m/Y H:i', strtotime('-3 hour', strtotime($devices[$i]->traccar->server_time)));
    ...
    public function testemapa()
    {
		...
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST,  2);
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('lat' => '-9.383721','lng' => '-40.520584','map' => 'CARSEG')));
		$data = curl_exec($ch);
		curl_close($ch);
		$data = json_decode($data);
