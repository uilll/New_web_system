...
$user = strip_tags((isset($_POST['user'])) ? $_POST['user'] : '');
...
if((int)$hora_ % 2 == 0){
	...
	chave3 = AIzaSyAJ2k-brcZe6MoXjcPYnQH6RJc-t2hITKE;
	chave4 = AIzaSyDeFzM5Mt0iwEOogWUPwg78twLD4Lm70qs;
} */
...
if(!empty($lat) && !empty($lng) && !$feito):
	if ($map=='google'){ 
		...
		$server = 'nominatim.carseg.com.br'; // or IP address https://nominatim.carseg.com.br/nominatim/status.php
		$url = 'https://' . $server . '/nominatim/reverse.php?format=json&lat=' . urlencode($lat) . '&lon=' . urlencode($lng);
		...
		if(!($user == 2 || $user == 3 || $user == 6)){
			if (false){//is_null($address_)){
				$json = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?latlng='.$lat.','.$lng.'&key='.$google_key);//AIzaSyBYs1o3hCH3BW2Fk_9Q3_maBuSeKelzZi8' );
				$location2 = process_google (json_decode($json));			
				$address_ = array_get($location2, 'address');
				//$state = array_get($location, 'state');
				if (is_null($address_)){
					$msg = $lat.",".$lng.",".$city."(auto-address) \r\n";
					$fp = fopen('/var/www/html/releases/20190129073809/public/report001.txt', "a+");
					fwrite($fp, $msg);
					fclose($fp);
				}
...
if (!empty($queries)){
	...
	$server = 'nominatim.carseg.com.br'; // or IP address
	$url = 'https://' . $server . '/nominatim/search.php?format=json&countrycodes=br&q=' . urlencode($queries) ;
	//echo $url;
	$contents = file_get_contents($url);
 ...
 function get_distance($lat_local,$lon_local,$city,$state){
	$server = 'nominatim.carseg.com.br'; // or IP address
	$url = 'https://' . $server . '/nominatim/search.php?format=json&city=' . urlencode($city) . '&state=' . urlencode($state);
	$contents = file_get_contents($url);
	$dec_contents = json_decode($contents);
	...
	$server = 'routes.carseg.com.br'; // or IP address 
