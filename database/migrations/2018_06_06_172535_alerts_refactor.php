...
class AlertsRefactor extends Migration
{
    ...
    private function getAlerts()
    {
        ...
        foreach ($alerts as $alert) {
            ...
            $drivers = DB::table('alert_driver_pivot')->where('alert_id', $alert->id)->get();
            if ($drivers) {
                $items[] = array_merge($_item, [
                    'type' => 'driver',
            ...
            $events_custom = DB::table('alert_event_pivot')->where('alert_id', $alert->id)->get();
            if ($events_custom) {
                $items[] = array_merge($_item, [
                    'type' => 'custom',
            ...
            $geofences = DB::table('alert_geofence')->where('alert_id', $alert->id)->get();
            if ($geofences) {
                $_geofences = [];
